<div class="container-fluid">
    <div class="d-flex justify-content-center align-items-center flex-column"> 
        ...
        <h4 class="fw-bold text-center text-truncate mt-2"> {{__('Tax Law Depreciation Expense Report')}}</h4>
    ...
    <div class="card shadow-sm border-0 mt-4">
        <div class="card-header pb-2">
            ...
            <div class="table-responsive mt-4"  style="overflow-x: auto; overflow-y: hidden;" wire:loading.remove wire:target="search, accountId, month"> 
                <table class="table table-striped table-hover align-middle mb-0" style="min-width: 1800px;">
                    <thead class="table-light">
                        <tr>
                            <th class="text-center border" rowspan="3" style="position: sticky; left: 0; z-index: 2; width: 60px;">{{ __('#') }}</th>
                            <th class="text-center border" rowspan="3">{{ __('Fixed Asset Class') }}</th>
                            <th class="text-center border" colspan="4">{{ __('Opening Balance') }}</th>
                            <th class="text-center border" colspan="6">{{ __('This Period') }}</th>
                            <th class="text-center border" rowspan="2" colspan="2">{{ __('Depreciation Base Value') }}</th>
                            <th class="text-center border" rowspan="3">{{ __('Rate') }}</th>
                            <th class="text-center border" rowspan="2" colspan="2">{{ __("Depreciation Expense (Depreciation charge)") }}</th> 
                            <th class="text-center border" rowspan="2" colspan="2">{{ __("Book Value (Depreciation charge)") }}</th>
                        </tr>
                        <tr>
                            <th class="text-center border" colspan="2">{{ __('Original Cost (History Cost)') }}</th>
                            <th class="text-center border" colspan="2">{{ __('Book Value (Underpreciated Value)') }}</th>
                            <th class="text-center border" colspan="2">{{ __('Pur. (Cost) = Fixed asset addition') }}</th>
                            <th class="text-center border" colspan="2">{{ __('Sell/Disposal') }}</th>
                            <th class="text-center border" colspan="2">{{ __('Adjustment') }}</th>
                        </tr>
                        <tr>
                            <th class="text-center border">{{ __('USD') }}</th>
                            <th class="text-center border">{{ __('KHR') }}</th>
                            <th class="text-center border">{{ __('USD') }}</th>
                            <th class="text-center border">{{ __('KHR') }}</th>
                            <th class="text-center border">{{ __('USD') }}</th>
                            <th class="text-center border">{{ __('KHR') }}</th>

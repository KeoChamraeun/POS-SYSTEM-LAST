<div class="container-fluid">  
    ...
    <div class="card shadow-sm border-0 mt-4">
        <div class="card-header pb-2">
            ...
            <div class="table-responsive mt-4" wire:loading.remove wire:target="search, companyId">
                <table class="table table-striped table-hover align-middle mb-0">
                    ...
                    <tbody>
                        @if (!count($users) > 0)
                            <tr>
                                <td colspan="10" class="text-center fw-semibold text-danger py-4">
                            ...
                            @foreach ($users as $index => $user)
                                <tr>
                                    ...
                                    <td class="d-flex align-items-center">
                                        <img src="{{ $user->profile ? asset($user->profile) : asset('assets/icon/profile.png') }}" class="thumb-md rounded-circle me-1">
                                        {{$user->name}}
                                    </td>
                                    <td class="text-start fw-semibold">{{ $user->username }}</td>
                                    <td class="text-start fw-semibold">{{ $user->role->name }}</td>
                                    ...
                                    <td class="text-start">
                                        <button type="button" class="btn btn-outline-{{$user->banned ? 'danger' : 'success'}} btn-sm px-3 py-1" wire:click="updateBanned({{ $user->id }})">
                                            {{$user->banned ? __('Banned') : __('Active')}}
                                        </button>
                                    </td>
                                    <td class="text-center align-middle">
                                        <a wire:click="restPasword({{ $user->id }})" class="rounded-pill btn btn-sm btn-outline-danger border-0 px-1 py-0">

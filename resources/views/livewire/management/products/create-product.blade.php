<div class="modal fade" id="createProductModal" tabindex="-1" aria-labelledby="createProductModalLabel" wire:ignore.self data-bs-backdrop="static">
        ...
        <div class="modal-content rounded-4 shadow">
            <div class="modal-header bg-primary rounded-top-4 px-4 py-3 border-bottom-0">
                <h5 class="modal-title fw-bold" id="createProductModalLabel">
                    <i class="fas fa-box-open me-2 text-dark"></i> {{ __('New Product') }}
                </h5>
...
document.addEventListener('livewire:init', () => {
    Livewire.on('open-create-product', () => {
        bootstrap.Modal.getOrCreateInstance(document.getElementById('createProductModal'))?.show();
    });
    Livewire.on('close-create-product', () => {
        bootstrap.Modal.getOrCreateInstance(document.getElementById('createProductModal'))?.hide();
    });
});
</script>